  ...
  return (
    ...
    >
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#F8FAFC', fontFamily: 'Poppins' }}>{title}</div>
...
// Derives ranked violation list from MOCK_CATEGORY_DATA — auto-updates when new categories added
const TOP_VIOLATIONS = MOCK_CATEGORY_DATA.map(d => ({
  type: d.name,
  count: Math.round(d.value * 13), // scale % → approximate case count
  pct: d.value,
  color: d.color,
}));
  ...
  return (
    <AppLayout>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 20 }}>
        {[
          { label: 'Avg. Confidence', value: `${avgConfidence}%`, color: '#22C55E', sub: 'AI detection accuracy' },
          { label: 'Peak Day',        value: 'Friday',             color: '#2563EB', sub: '68 violations avg.' },
          { label: 'Top Violation',   value: 'Helmetless Riding',  color: '#F97316', sub: '28% of all cases' },
          { label: 'Resolution Rate', value: '78.4%',              color: '#06B6D4', sub: 'Cases closed' },
        ].map(s => (
          <div key={s.label} className="glass-card" style={{ padding: '18px 20px' }}>
            ...
            <div style={{ fontSize: 12.5, color: '#F8FAFC', fontWeight: 600 }}>{s.label}</div>
        ...
        <ChartCard title="Daily Violation Trend" subtitle="Last 7 days breakdown by type">
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={MOCK_DAILY_TREND}>
              ...
              <Bar dataKey="helmets"   name="Helmetless" fill="#F97316" radius={[3,3,0,0]} stackId="a" />
              <Bar dataKey="signals"   name="Signal"     fill="#EF4444" radius={[0,0,0,0]} stackId="a" />
              <Bar dataKey="noParking" name="No-Parking" fill="#8B5CF6" radius={[0,0,0,0]} stackId="a" />
              <Bar dataKey="zebra"     name="Zebra"      fill="#F59E0B" radius={[0,0,0,0]} stackId="a" />
      ...
      {/* Monthly trend - full width */}
        <ChartCard title="Monthly Violation Statistics" subtitle="Jan–Aug 2024 — Detected vs Resolved + new categories">
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={MOCK_MONTHLY_TREND}>
              ...
              <Tooltip content={<CustomTooltip />} />
              <Legend wrapperStyle={{ fontSize: 10, color: '#94A3B8' }} />
              <Area type="monotone" dataKey="violations" name="Detected"   stroke="#2563EB" fill="url(#blueGrad2)" strokeWidth={2} dot={false} />
              <Area type="monotone" dataKey="resolved"   name="Resolved"   stroke="#22C55E" fill="url(#greenGrad)" strokeWidth={2} dot={false} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: 20, marginTop: 20 }}>
              ...
              <div key={v.type}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 5 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 13 }}>{VIOLATION_META[v.type]?.icon ?? '⚠️'}</span>
                    <span style={{ color: '#F8FAFC', fontWeight: 500 }}>{v.type}</span>
                  </div>
                  <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                    <span style={{ color: '#64748B' }}>{v.count} cases</span>
        ...
        <ChartCard title="Detection Accuracy" subtitle="AI model performance metrics">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {[
              { label: 'Overall Accuracy', value: 94.7, color: '#22C55E' },
              { label: 'Precision Score', value: 92.3, color: '#2563EB' },
              { label: 'Recall Score', value: 96.1, color: '#06B6D4' },
              { label: 'F1 Score', value: 94.2, color: '#8B5CF6' },
              { label: 'False Positive Rate', value: 5.3, color: '#F97316' },
            ].map((m, i) => (
              <div key={m.label}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 5 }}>
